...
export function RaffleList() {
    ...
    const handleBuyTicket = async (raffleAddress: string, price: bigint) => {
        if (!signer) {
        ...
        try {
            ...
            const tx = await raffleWithSigner.buyTicket({
                value: price,
                gasLimit: 1000000
            });
...
function RaffleCard({
    ...
    let displayCost = `${ethCost} ETH`;
    let quote = "";

    if (selectedAsset === "USDC" && prices.eth > 0) {
        const costInUSD = ethCost * prices.eth;
    ...
    const handleClick = async () => {
        ...
        try {
            if (selectedAsset === "ETH") {
                await onBuy(raffle.id, raffle.rawEntranceFee);
            } else {
